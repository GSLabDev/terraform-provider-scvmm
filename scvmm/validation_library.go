...
func validateVirtualMachineName(v interface{}, k string) (warnings []string, errors []error) {
	virtualMachineName := v.(string)
	match, err := regexp.MatchString("^[^*?:<>|/[\\]]+$", virtualMachineName)
	if err != nil {
...
func validateTemplateName(v interface{}, k string) (warnings []string, errors []error) {
	...
	match, err := regexp.MatchString("^[^*?:<>|/[\\]]+$", templateName)
	if err != nil {
		return returnError("Regex error: Please report bug to terraform", err)
	}
	if !match {
...
func validateVMMServer(v interface{}, k string) (warnings []string, errors []error) {
	vmmServerName := v.(string)
	match, err := regexp.MatchString("^[\\w\\d.\\-_]+$", vmmServerName)
	if err != nil {
		return returnError("Regex error: Please report bug to terraform", err)
	}
	if !match {
...
func validateCloudName(v interface{}, k string) (warnings []string, errors []error) {
	cloudName := v.(string)
	match, err := regexp.MatchString("^[\\w\\s\\d.\\-_]+$", cloudName)
	if err != nil {
		return returnError("Regex error: Please report bug to terraform", err)
	}
	if !match {
